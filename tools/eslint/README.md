...
```bash
npm install @draco-china/eslint eslint -D
```
...
module.exports = {
  // index 可省略
  ...require('@draco-china/eslint/javascript/{index, node, rax, react, vue}'),
...
module.exports = {
  // index 可省略
  ...require('@draco-china/eslint/typescript{index, node, rax, react, vue}'),
  ignorePatterns: ['.eslintrc.js'],
};
```
...
- 了解如何在继承本包的基础上对项目 ESLint 进行个性化配置，可参考官网的 [Configuring ESLint](https://eslint.org/docs/user-guide/configuring)。下面简介下 ESLint 配置中的几个常用字段：
  - `extends`: 继承一组规则集。`"extends": ["@draco-china/eslint"],` 表示继承本包定义的规则配置。
